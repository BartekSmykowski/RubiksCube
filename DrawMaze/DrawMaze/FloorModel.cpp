...
void FloorModel::Create(glm::vec3 wspolzedne, colorT type)
{
	GLuint vao;
	GLuint vbo;
	GLuint ibo;
	...
	std::vector<unsigned int> indices = { 0, 1, 2, 0, 2, 3, //down
	};


	//down
	vertices.push_back(VertexFormat(glm::vec3(mjeden + i_t, mjeden + j_t, ulamek + k_t), glm::vec2(1, 0)));
	vertices.push_back(VertexFormat(glm::vec3(ulamek + i_t, mjeden + j_t, ulamek + k_t), glm::vec2(0, 0)));
	...
	const glm::mat4& view_matrix)
{
	...
	glBindTexture(GL_TEXTURE_2D, this->GetTexture("FloorTexture"));
